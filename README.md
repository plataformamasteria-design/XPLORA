...
Abra `http://127.0.0.1:8787`. Sem `PANEL_PASSWORD`, o servidor aceita somente
conexões locais; em uma hospedagem pública, defina usuário e senha.
...
- O painel executa sem limite de duração (`MEET_MAX_MINUTES=0`). Apenas um robô de reunião pode ficar ativo por vez, evitando gravações concorrentes na mesma saída de áudio.
...
- Após a reunião, clique em **Buscar arquivos**, selecione a gravação e use **Transcrever**. Os resultados `.txt`, `.srt` e `.json` ficam ao lado do áudio e podem ser baixados pelo painel.

O aviso **Ambiente pronto** confirma que Chromium, PulseAudio, FFmpeg, armazenamento
e Whisper estão disponíveis. Se algum item falhar, o envio fica bloqueado e o painel
mostra qual dependência precisa ser corrigida.
